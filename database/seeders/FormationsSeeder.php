...
class FormationsSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // --- 2. Catégories réelles (voir CategorieSeeder — lancé ici
        // aussi par sécurité si ce seeder est exécuté seul, firstOrCreate
        // évite tout doublon dans les deux cas) ---
        $this->call(CategorieSeeder::class);

        $categoriesParTitre = Categorie::pluck('id', 'titre');
        ...
        foreach ($formations as $data) {
            $categorieTitre = $data['categorie'];
            unset($data['categorie']);

            Formation::create(array_merge($data, [
                'categorie_id' => $categoriesParTitre[$categorieTitre] ?? $categoriesParTitre->first(),
                'user_id' => $owner->id,
                'nb_inscrit' => 0,
            ]));
        }
